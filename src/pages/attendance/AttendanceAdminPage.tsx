...
import { ATTENDANCE_STATUS_LABELS, ATTENDANCE_STATUS_OPTIONS } from '@/constants/enums';
import { AttendanceUpsertStatusRequest, AttendanceStatus } from '@/api/generated/apiClient';
...
export default function AttendanceAdminPage() {
  ...
  // Filters
  const [branchId, setBranchId] = useState(user?.role === 'HQ_ADMIN' ? '' : (user?.branchId || ''));
  ...
  // Lookups
  const { data: users = [] } = useQuery({ queryKey: ['usersLookup'], queryFn: getUsersOnce });
  const { data: branches = [] } = useQuery({ queryKey: ['branchesLookup'], queryFn: getBranchesOnce });
  ...
  const { data: todayData, isLoading: todayLoading } = useQuery({
    ...
    queryFn: () => attendanceRepo.today(branchId || undefined, employeeUserId || undefined, page, pageSize),
  ...
  const { data: monthData, isLoading: monthLoading } = useQuery({
    ...
    queryFn: () => attendanceRepo.employeeMonth(employeeUserId, year, month, branchId || undefined),
    enabled: activeTab === 'month' && !!employeeUserId,
  });

  const upsertMutation = useMutation({
    mutationFn: (req: AttendanceUpsertStatusRequest) => attendanceRepo.upsertStatus(req),
    onSuccess: () => {
  ...
  const handleEditStatus = (record: any) => {
    let workDate = record.workDate ? new Date(record.workDate).toISOString().split('T')[0] : '';
    let status = record.status || AttendanceStatus._1;
    let checkInAt = record.checkInAt ? new Date(record.checkInAt).toISOString().slice(0, 16) : '';
    let checkOutAt = record.checkOutAt ? new Date(record.checkOutAt).toISOString().slice(0, 16) : '';
    let note = record.note || '';

    const renderModal = () => openModal('Edit Attendance Status', (
      <ModalContent
        footer={
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
            <Button variant="secondary" onClick={closeModal}>Cancel</Button>
            <Button onClick={() => {
              upsertMutation.mutate(new AttendanceUpsertStatusRequest({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 600, color: 'var(--c-text)' }}>Attendance Management</h1>

      <Card>
        <div style={{ padding: '16px', display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'flex-end' }}>
          {user?.role === 'HQ_ADMIN' && (
          ...
          <div style={{ width: '250px' }}>
            <Select label="Employee" options={employeeOptions} value={employeeUserId} onChange={(e) => setEmployeeUserId(e.target.value)} />
          </div>
      ...
      <div style={{ display: 'flex', gap: '2px', borderBottom: '1px solid var(--c-border)' }}>
        <button
          ...
          style={{
            ...
            border: 'none',
            borderBottom: activeTab === 'today' ? '2px solid var(--c-primary)' : '2px solid transparent',
            color: activeTab === 'today' ? 'var(--c-primary)' : 'var(--c-muted)',
        ...
        <button
          ...
          style={{
            ...
            background: 'transparent',
            border: 'none',
            borderBottom: activeTab === 'month' ? '2px solid var(--c-primary)' : '2px solid transparent',
            color: activeTab === 'month' ? 'var(--c-primary)' : 'var(--c-muted)',
        ...
        <Card>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--c-border)', textAlign: 'left' }}>
                  <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>Employee</th>
                  <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>Date</th>
                  <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>Check-in</th>
                  <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>Check-out</th>
                  ...
                  todayItems.map((rec) => (
                    <tr key={rec.id} style={{ borderBottom: '1px solid var(--c-border)' }}>
                      ...
                      <td style={{ padding: '16px' }}>
                        {rec.workDate ? new Date(rec.workDate).toLocaleDateString() : '-'}
                      </td>
                      <td style={{ padding: '16px' }}>
                      ...
                      <td style={{ padding: '16px' }}>
                        <span style={{
                          fontSize: '12px', padding: '2px 8px', borderRadius: '12px', background: 'var(--c-bg-alt)', border: '1px solid var(--c-border)',
                          color: rec.status === 1 ? 'var(--c-success)' : 'var(--c-text)'
                        }}>
                          {rec.status ? ATTENDANCE_STATUS_LABELS[rec.status as unknown as number] : '-'}
                        </span>
                      </td>
                      ...
                      <td style={{ padding: '16px', textAlign: 'right' }}>
                        <Button variant="secondary" size="sm" onClick={() => handleEditStatus(rec)} title="Edit Status">
                          <Edit size={16} />
                        </Button>
                      </td>
                    </tr>
          ...
          <div style={{ padding: '16px', borderTop: '1px solid var(--c-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <Button variant="secondary" size="sm" disabled={page >= todayTotalPages} onClick={() => setPage(p => p + 1)}>
      ...
      {activeTab === 'month' && (
        <Card>
          <div style={{ padding: '16px', borderBottom: '1px solid var(--c-border)', display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
            <div style={{ width: '120px' }}>
              <Select label="Year" options={years} value={year} onChange={(e) => setYear(Number(e.target.value))} />
            </div>
                ...
                <tbody>
                  {monthLoading ? (
                    ...
                    monthRecords.map((rec, idx) => (
                      <tr key={rec.id || idx} style={{ borderBottom: '1px solid var(--c-border)' }}>
                        <td style={{ padding: '16px' }}>
                          {rec.workDate ? new Date(rec.workDate).toLocaleDateString() : '-'}
                        </td>
                        <td style={{ padding: '16px' }}>
