...
import { AttendanceCheckInRequest, AttendanceCheckOutRequest } from '@/api/generated/apiClient';
...
export default function MyAttendancePage() {
  ...
  const checkOutMutation = useMutation({
    mutationFn: (req: AttendanceCheckOutRequest) => attendanceRepo.checkOut(req),
    onSuccess: () => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <Card>
        <div style={{ padding: '16px', borderBottom: '1px solid var(--c-border)' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--c-text)' }}>Today's Attendance</h2>
        </div>
        <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <label style={{ fontSize: '14px', fontWeight: 500, color: 'var(--c-text)' }}>Note (Optional)</label>
            <textarea
              value={note}
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <Button
              ...
              style={{ flex: 1 }}
            >
              {checkInMutation.isPending ? <Loader2 size={18} className="animate-spin" /> : <LogIn size={18} style={{ marginRight: '8px' }} />}
              Check-in
            </Button>
            <Button
              ...
              style={{ flex: 1 }}
            >
              {checkOutMutation.isPending ? <Loader2 size={18} className="animate-spin" /> : <LogOut size={18} style={{ marginRight: '8px' }} />}
              Check-out
            </Button>
          </div>
        </div>
      ...
      <Card>
        <div style={{ padding: '16px', borderBottom: '1px solid var(--c-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 600, color: 'var(--c-text)' }}>Monthly View</h2>
          <div style={{ display: 'flex', gap: '12px' }}>
