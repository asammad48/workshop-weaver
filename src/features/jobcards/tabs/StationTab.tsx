...
export const StationTab: React.FC<StationTabProps> = ({ jobCardId }) => {
  ...
  const moveMutation = useMutation({
    mutationFn: (body: MoveJobCardRequest) => stationsRepo.move(jobCardId, body),
    onSuccess: (res) => {
      if (res.success) {
        toast.success("Job card moved successfully");
        queryClient.invalidateQueries({ queryKey: ["stationHistory", jobCardId] });
  ...
  if (isError) {
    return (
      <div style={{ padding: '48px', textAlign: 'center', color: 'var(--c-danger)' }}>
        ...
        <p>Error loading history: {(error as any)?.message || 'Unknown error'}</p>
      </div>
    );
  }
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--c-border)', textAlign: 'left' }}>
                <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>From Station</th>
                <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>To Station</th>
                <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>Moved At</th>
                <th style={{ padding: '16px', color: 'var(--c-muted)', fontSize: '14px', fontWeight: 500 }}>Moved By</th>
                ...
                history.map((item: any) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--c-border)', transition: 'background-color 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'rgba(var(--c-primary-rgb), 0.02)'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    ...
                    <td style={{ padding: '16px', color: 'var(--c-text)' }}>
                      {item.movedAt ? new Date(item.movedAt).toLocaleString() : "-"}
                    </td>
                    <td style={{ padding: '16px', color: 'var(--c-text)' }}>{(item as any).movedByName || '-'}</td>
                    <td style={{ padding: '16px', color: 'var(--c-text)' }}>{item.notes || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
